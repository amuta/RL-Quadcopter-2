...
class Critic:
    ...
    def build_model(self):
        ...
        net_states = layers.Dense(units=48, activation='relu')(states)
        net_states = layers.Dense(units=24, activation='relu')(net_states)

        # Add hidden layer(s) for action pathway
        net_actions = layers.Dense(units=48, activation='relu')(actions)
        net_actions = layers.Dense(units=24, activation='relu')(net_actions)
        ...
        Q_values = layers.Dense(units=1, name='q_values',
                                kernel_initializer=initializers.RandomUniform(minval=-1e-5, maxval=1e-5))(net)
